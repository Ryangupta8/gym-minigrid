from gym_minigrid.multi_agent_minigrid import *
...
class MultiAgentEnv(MultiMiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        for i_obst in range(self.n_obstacles-1):
            ...
            init_pos = self.place_obj_trajectory(self.obstacles[i_obst], max_tries=100)
            self.obs_initpos.append(init_pos)

        self.obstacles.append(Ball(color='blue'))
        init_pos = self.place_obj_trajectory(self.obstacles[self.n_obstacles-1], max_tries=100)
        self.obs_initpos.append(init_pos)
    ...
    def step(self, action, action2):
        ...
        if action == self.actions.forward and not_clear:
            ...
            print("agent1 collision with objects")
            return obs, reward, done, info

        if action2 == self.actions.forward and not_clear2:
            reward = -1
            done = False
            print("agent2 collision with objects")
            return obs, reward, done, info
        ...
        for i_obst in range(len(self.obstacles)):
            ...
            try:
                ...
                obs_pos=self.place_obj_trajectory(self.obstacles[i_obst], max_tries=100)
